...
module.exports = grammar({
  ...
  rules: {
    ...
    // prec(-1) gives line_comment lower lexer priority than string_literal_fragment
    // (prec(1)).  When `#` appears inside a string both tokens would otherwise
    // match the same span (e.g. `#8B0000`), and the comment would win by default.
    // The regex is also restricted to `[^"\n]*` so the comment token never
    // produces a *longer* match than the string fragment by consuming the
    // closing `"`.  At real comment positions nothing with higher priority is
    // valid, so line_comment still wins there.
    ...
    string: ($) =>
      seq(
        '"',
        repeat(
          choice($.string_literal_fragment, $.escape_sequence, $.interpolation),
        ),
        '"',
      ),

    // prec(1) beats line_comment's prec(-1) so that when both tokens match
    // the same span of characters (e.g. `#FF0000` right after the opening `"`)
    // the string fragment wins.  The line_comment regex is also restricted to
    // `/[^"\n]*/` so it never produces a *longer* match than this rule.
    string_literal_fragment: (_) => token(prec(1, /[^\\{"]+/)),
    ...
    map_expr: ($) =>
      seq(
        ":{",
        commaSep(seq(field("key", $._expr), ":", field("value", $._expr))),
        optional(","),
        "}",
      ),
    ...
    unary_expr: ($) =>
      prec(
        11,
        seq(
          field("operator", alias(choice("!", "not", "-"), $.unary_operator)),
          field("operand", $._expr),
        ),
      ),
    ...
    binary_expr: ($) =>
      choice(
        ...
        prec.left(
          ...
          seq(
            field("left", $._expr),
            field("operator", alias(choice("+", "-"), $.binary_operator)),
            field("right", $._expr),
          ),
        ),
        ...
        prec.left(
          ...
          seq(
            field("left", $._expr),
            field("operator", alias(choice("and", "&&"), $.binary_operator)),
            field("right", $._expr),
          ),
        ),
        ...
        prec.left(
          ...
          seq(
            field("left", $._expr),
            field("operator", alias(choice("or", "||"), $.binary_operator)),
            field("right", $._expr),
          ),
        ),
      ),
    ...
    assignment: ($) =>
      seq(field("target", $.identifier_path), "=", field("value", $._expr)),
    ...
    _match_pattern: ($) => choice($.wildcard, $._literal, $.identifier_path),
    ...
    menu_option: ($) => seq(field("label", $.string), field("body", $.block)),
    ...
    event_constraint: ($) =>
      seq("<", "event", ":", field("kind", choice("dialogue", "choice")), ">"),
